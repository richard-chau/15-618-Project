...
def prep(tup, num_workers, blockMovies):
    #data = tup[1]
    key = None
    V_sub_pre_list = [[] for i in range(num_workers)]
    for item_ in tup:
        item = item_[1]
        key = item_[0]
        V_sub_pre_list[int(item[1] / blockMovies)].append(item)#(item_)
    return [(key, V_sub_pre_list)]
    #return V_sub_pre_list

V_sub_pre = V.mapPartitions(lambda x: prep(x, num_workers, blockMovies)).persist()

dummy = 1
print(dummy)
def updateParameter1(tup, n, samplingCnt):
    ...
    if len(V_sub) == 1:
        V_sub = list(V_sub)[0]
    if len(V_sub) != 0:
        sampling = np.random.choice(len(V_sub), samplingCnt)
...
for itr in range(iter_n):
    
    t_begin = time.process_time()
    strata = np.random.permutation(num_workers)
    inv_strata = [0] * num_workers
    for i in range(len(strata)):
        inv_strata[strata[i]] = i
    #V_subs = V.filter(lambda x: strata[int(x[1][1]/blockMovies)] == x[0]) #[node, (user, movie,  vec)]
    #V_sub_pre = V.mapPartitions(lambda x: prep(x, num_workers, blockMovies))
    V_subs = V_sub_pre.map(lambda x: (x[0], x[1][inv_strata[x[0]]] ), preservesPartitioning=True )
    H_subs = H.keyBy( lambda x: strata[int(x[0] / blockMovies)])#.partitionBy(num_workers) #[node, (movie, (N, vec))] 
    ...
    argsOutput = argsInput.map(lambda x: updateParameter1(x, iter_n, (int)(totalData/num_workers/num_workers) ), preservesPartitioning=True).persist() #[node, (Wdict, Hdict)]
